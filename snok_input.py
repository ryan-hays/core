...
def read_receptor_and_ligand(filename_queue, epoch_counter, train):
    ...
    def count_frame_from_epoch(epoch_counter, ligand_labels, train):
        ...
        def select_pos_frame():
            return tf.constant(0)

        def select_neg_frame():
            return tf.mod(tf.div(1 + epoch_counter, 2), tf.shape(ligand_labels) - 1) + 1
    ...
    return ligand_file, tf.squeeze(epoch_counter), tf.squeeze(label), ligand_elements, tf.squeeze(
        ligand_coords), receptor_elements, tf.squeeze(multiframe_receptor_coords)


def convert_protein_and_ligand_to_image(ligand_elements, ligand_coords, receptor_elements, receptor_coords, side_pixels,
                                        pixel_size):
...
def image_and_label_queue(batch_size, pixel_size, side_pixels, num_threads, filename_queue, epoch_counter, train=True):
    ...
    multithread_batch = tf.train.batch([ligand_file, current_epoch, label, sparse_image_4d], batch_size,
