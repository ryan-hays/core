...
def index_the_database(database_path):
    ...
    ligand_file_list = ['../datasets/labeled_av4/4aje/4aje_1325_ligand.av4' for i in range(100)]
    receptor_file_list = ['../datasets/labeled_av4/4aje/4aje.av4' for i in range(100)]
...
def read_receptor_and_ligand(filename_queue):
    ...
    #print sess.run([filename_queue[1],filename_queue[2]])
    sess.close()
...
def convert_protein_and_ligand_to_image(ligand_elements,ligand_coords,receptor_elements,receptor_coords,side_pixels,pixel_size):
    ...
    centered_ligand_coords = ligand_coords
    centered_receptor_coords = receptor_coords

    identity = np.zeros((4,4),dtype='float32')
    for i in range(4):
        for j in range(4):
                if i == j:
                    identity[i][j] = 1
    ...
    def generate_transition_matrix(attempt, transition_matrix, batch_of_transition_matrices):
        ...
        transition_matrix= tf.Variable(initial_value = identity)
        attempt += 1
    ...
    uncropped_coords = tf.concat(0,[ceiled_ligand_coords,ceiled_receptor_coords])
    uncropped_elements = tf.concat(0,[ligand_elements+10,receptor_elements])

    test_sparse_complex = tf.SparseTensor(indices=uncropped_coords,values=uncropped_elements, shape=[100,100,100])
    test_dense_complex = tf.sparse_tensor_to_dense(test_sparse_complex,validate_indices=False)
...
def image_and_label_queue(sess,batch_size,pixel_size,side_pixels,num_threads,database_path):
    ...
    #f = open('pdbs.txt','w')
    #f.write(str(ligand_file_list))
    #f.write('\n')
    #f.write(str(receptor_file_list))
    #f.close()
    ...
    #l_name,r_name=filename_queue[1],filename_queue[2]
