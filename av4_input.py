...
def convert_protein_and_ligand_to_image(ligand_elements,ligand_coords,receptor_elements,receptor_coords,side_pixels,pixel_size):
    ...
    def generate_transition_matrix(attempt,cameraview,random_cameraviews):
        """Takes initial coordinates of the ligand, generates a random affine transform matrix and transforms coordinates."""
        cameraview= tf.gather(random_cameraviews,tf.random_uniform([], minval=0, maxval=affine_transform_pool_size-1, dtype=tf.int32))
        attempt += 1
        return attempt,cameraview,random_cameraviews

    def not_all_in_the_box(attempt,cameraview,random_cameraviews,ligand_coords=centered_ligand_coords,box_size=box_size,max_num_attempts=max_num_attemts):
        """Takes affine transform matrix and box dimensions, performs the transformation, and checks if all atoms
        are in the box."""
        transformed_coords,cameraview = affine_transform(ligand_coords,cameraview)
    ...
    rotatated_ligand_coords,_ = affine_transform(centered_ligand_coords,final_cameraview)
    rotated_receptor_coords,_ = affine_transform(centered_receptor_coords,final_cameraview)
...
#    complex_image = tf.sparse_tensor_to_dense(sparse_complex,validate_indices=False)
    ...
    #FIXME: remove hydrogens
    # matmul is to escape bug within tensorflow when a boolean mask of undefined shape can not be used
#    ligand_hydrogen_mask = tf.cast(tf.matmul(tf.transpose([ligand_elements], perm=[1, 0]), tf.ones([1, 1], tf.int32))[:, 0] > 1, tf.bool)
#    ligand_elements_noh = tf.boolean_mask(ligand_elements,ligand_hydrogen_mask)
#    ligand_coords_noh = tf.boolean_mask(ceiled_ligand_coords,ligand_hydrogen_mask)
#
#    receptor_hydrogen_mask = tf.cast(tf.matmul(tf.transpose([cropped_receptor_elements], perm=[1, 0]), tf.ones([1, 1], tf.int32))[:, 0] > 1, tf.bool)
#    receptor_elements_noh = tf.boolean_mask(cropped_receptor_elements,receptor_hydrogen_mask)
#    receptor_coords_noh = tf.boolean_mask(cropped_receptor_coords,receptor_hydrogen_mask)
#
    ...
    # FIXME: merge receptor an ligand and sort coordinates in sparse tensor
#    complex_coords = tf.concat(0,[ligand_coords_noh,receptor_coords_noh])
#    complex_elements = tf.concat(0,[ligand_elements_noh+7,receptor_elements_noh])


    # FIXME: sort coordinates in a sparse tensor
    sparse_complex = tf.SparseTensor(indices=complex_coords, values=tf.to_float(complex_elements),dense_shape=[side_pixels, side_pixels, side_pixels])
    ordered_sparse_complex = tf.sparse_reorder(sparse_complex)
    complex_image = tf.sparse_tensor_to_dense(ordered_sparse_complex,validate_indices=False)


    # FIXME: calculating all ligand atomic pairwise disntances
    pairwise_atomwise_distances_3d = tf.reshape(tf.reduce_sum(tf.square(tf.transpose(tf.expand_dims(ligand_coords, 2), perm=[0, 2, 1]) - ligand_coords),reduction_indices=2),[-1])**0.5
    atomwise_distances_no_0 = tf.boolean_mask(pairwise_atomwise_distances_3d,pairwise_atomwise_distances_3d>tf.constant(0.0))
    sorted_bonds = -tf.nn.top_k(-atomwise_distances_no_0,k=tf.shape(atomwise_distances_no_0)[0]).values

    return complex_image,ligand_center_of_mass,final_cameraview
