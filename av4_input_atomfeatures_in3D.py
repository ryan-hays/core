...
#def single_dense_image_example(sess,pixel_size,side_pixels,database_path,num_epochs):
...
filename_queue, examples_in_database = index_the_database_into_queue("../datasets/labeled_av4", shuffle=False)
...
ligand_file, current_epoch, label, ligand_elements, ligand_coords, receptor_elements, receptor_coords = read_receptor_and_ligand(
    filename_queue, 10000, 1000)

# convert coordinates of ligand and protein into an image
dense_complex, ligand_center_of_mass, final_transition_matrix = convert_protein_and_ligand_to_image(ligand_elements,
                                                                                                    ligand_coords,
                                                                                                    receptor_elements,
                                                                                                    receptor_coords,
                                                                                                    side_pixels=20,
...
# now try slplitting ligand elements
# one of the ways is to cut off elements that are close for each of them # and apply a sparse mask

#near = tf.reduce_sum(tf.square(ligand_coords[0]-ligand_coords),reduction_indices=1) < 4**2
#test_shape = tf.squeeze(tf.shape(ligand_elements))
#test_split = tf.split(split_dim=0,num_split=test_shape[0],value=ligand_elements)
#test_split_v = tf.split_v(ligand_elements,13,split_dim=0)
#test_unpack = tf.unpack(ligand_elements)
#test_arrange = tf.range(test_shape)
...
pairwise_atomwise_distances_3d = tf.transpose(tf.expand_dims(ligand_coords,2),perm=[0,2,1]) -ligand_coords
# get ceiled coordinates for sparse tensor in 4D
ceiled_distance_3d = tf.cast(tf.round(pairwise_atomwise_distances_3d),dtype=tf.int64)
fourth_index_1d = tf.range(tf.shape(ligand_elements)[0])
fourth_index_2d = tf.reshape(tf.tile(fourth_index_1d,tf.shape(ligand_elements)),[tf.squeeze(tf.shape(ligand_elements)),tf.squeeze(tf.shape(ligand_elements))])

ceiled_4d = tf.concat(2,[ceiled_distance_3d,tf.expand_dims(tf.cast(fourth_index_2d,tf.int64),2)])


# now make a sparse tensor
#tensor_values = tf.ones()
sparse_4d = ceiled_4d

# now use sparse retain on this tensor to save what we want

#ceiled_4d = tf.pack(2,[ceiled_distance_3d,ceiled_distance_3d])
# concatenate this fourth index with our tensor
#tf.concat()

# big_sparse_tensor = tf.SparseTensor()

# result can be a four-dimensional dense tensor of the length=number_of_atoms

# get ceiled coordinates of ligand atoms that would fit
# ceiled_ligand_coords = tf.cast(tf.round(-0.5 + (tf.cast(side_pixels,tf.float32)*0.5) + (rotatated_ligand_coords/pixel_size)),tf.int64)
# ceiled_receptor_coords = tf.cast(tf.round(-0.5 + (tf.cast(side_pixels, tf.float32) * 0.5) + (rotated_receptor_coords/pixel_size)),tf.int64)


# get a 2D mask num_atoms, num_atoms
#boolean_mask_2d = tf.reduce_max(,reduction_indices=2)


while True:
    #print sess.run(tf.shape(test_transpose))
    #print sess.run(tf.shape(ligand_coords))
    print sess.run(fourth_index_2d)
    time.sleep(2)