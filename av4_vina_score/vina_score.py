...
class evaluation:
    ...
    def debug_info(self, message):
        if FLAGS.debug == 'print':
            print message
        elif FLAGS.debug == 'log':
            with open(FLAGS.log, 'a') as fout:
                fout.write(message + '\n')
        elif FLAGS.debug == 'off':
            pass
    ...
    def eval_inter(self):
        ...
        '''
        distance = lambda i, j: np.sqrt(np.sum(np.power(self.lig[i].coords - self.rec[j].coords, 2)))
...
def main():
    v = evaluation(FLAGS.receptor, FLAGS.ligand)
    v.eval()
