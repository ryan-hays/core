...
def train():
    ...
    _,current_epoch,label_batch,sparse_image_batch,ligand_image_batch,ligand_elements = image_and_label_queue(batch_size=FLAGS.batch_size, pixel_size=FLAGS.pixel_size,
    ...
    x_atom_batch = tf.sparse_tensor_to_dense(ligand_image_batch,validate_indices=False)

    batches = []
    for i in range(8):
        batches.append(tf.boolean_mask(tf.cast(x_atom_batch,tf.float32),tf.equal(ligand_elements,i)))

    outputs = [0]
    for i in range(1,8):
        outputs.append(fast_net(batches[i],i,0.5))
        
    #TODO: take outputs, and modify image_batch using the outputs
    #perhaps use boolean masks
