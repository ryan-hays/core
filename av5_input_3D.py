...
import os,time,math
from av5_utils import generate_deep_affine_transform,affine_transform
...
number_of_ligand_atoms = sess.run(tf.shape(ligand_coords)[0])                                                                                                             
number_of_receptor_atoms = sess.run(tf.shape(receptor_coords)[0])                                                                                                         
...
#print sess.run([tf.shape(ligand_indices),tf.shape(receptor_indices)])
...
#print sess.run(tf.shape(receptor_elements))
...
top_filter = tf.reduce_max(tf.split_v(ceiled_complex_coords,[3,1],2)[0],reduction_indices=2)<side_pixels                                                                 
bottom_filter = tf.reduce_min(tf.split_v(ceiled_complex_coords,[3,1],2)[0],reduction_indices=2)>0
retain_atoms = tf.logical_and(top_filter,bottom_filter)
...
test = tf.to_int64(cropped_complex_coords)
shapes: = [tf.shape(ligand_coords)[0],tf.shape(receptor_coords)[0],4]
#sparse_tensor_of_images = tf.SparseTensor(indices=tf.to_int64(cropped_complex_coords),values=cropped_complex_elements,
 #                                         shape=[tf.shape(ligand_coords)[0],tf.shape(receptor_coords)[0],4])

i = 0
while i < 1:
    #print sess.run(tf.shape(test_transpose))
    #print (number_of_ligand_atoms)
    print sess.run(tf.shape(tiled_coord_shifts))
    print sess.run(tf.shape(tiled_receptor_coords))
    print sess.run(tf.shape(shifted_complex_coords))
    print sess.run(cropped_complex_coords[0])
    print sess.run(cropped_complex_coords[1])
    print sess.run(tf.shape(cropped_complex_coords))
    print sess.run(shapes)
    #print sess.run(complex_elements[1][1])
    i += 1
    time.sleep(2)
