...
def max_net(x_image_batch,keep_prob):
    ...
    h_conv3 = conv_layer(layer_name="conv3_2x2x2", input_tensor=h_pool2, filter_size=[2, 2, 2, 45, 60])
    ...
    h_conv4 = conv_layer(layer_name="conv4_2x2x2", input_tensor=h_pool3, filter_size=[2, 2, 2, 60, 75])
    ...
    h_conv5 = conv_layer(layer_name="conv5_2x2x2", input_tensor=h_pool4, filter_size=[2, 2, 2, 75, 90])
    ...
    with tf.name_scope("flatten_layer"):
        h_pool2_flat = tf.reshape(h_pool5, [-1, 10 * 10 * 10 * 90])
...
def compute_weighted_cross_entropy_mean(logits, labels,batch_size):
    ...
    with tf.name_scope('weighted_cross_entropy_mean'):  # TODO (multiclass) now hardcoded to two classes
        ...
        weighted_cross_entropy = tf.nn.weighted_cross_entropy_with_logits(logits, batch_y, pos_weight=50, name=None)
...
def train():
    ...
    with tf.name_scope('evaluate_predictions'):
        ...
        shuffled_cross_entropy_mean = compute_weighted_cross_entropy_mean(y_conv,shuffled_y_,FLAGS.batch_size)
