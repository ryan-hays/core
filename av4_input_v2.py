...
def index_the_database_into_queue(database_path, shuffle):
    ...
    def crystal_path(ligand_path):
        ...
        crystal_file_path = os.path.join('/home/ubuntu/xiao/data/newkaggle/dude/crystal/crystal_ligands', receptor,
                                         receptor + '_crystal.av4')
...
def convert_protein_and_multiple_ligand_to_image(ligand_elements, multiple_lgiand_coords, crystal_ligand_coords,
    ...
    def not_enough_in_the_box(attempt, transition_matrix, batch_of_transition_matrices,
                              centered_crystal_ligand=centered_crystal_ligand,
                              multiple_ligand_coords=centered_multiple_ligand_coords, box_size=box_size,
                              max_num_attempts=max_num_attempts):
        def affine_multiple_transform(index, multiple_coordinates=multiple_ligand_coords,
        ...
        not_all = tf.cast(
            tf.reduce_max(tf.cast(tf.square(box_size * 0.5) - tf.square(transformed_crystal_coords) < 0, tf.int32)),
            tf.bool)
        ...
        out_of_box_atoms = tf.squeeze(tf.reduce_sum(
            tf.cast(tf.square(box_size * 0.5) - tf.cast(tf.square(transformed_coords), tf.float32) < 0, tf.int32),
            reduction_indices=-1))
    ...
    rotatated_ligand_coords = tf.map_fn(affine_multiple_transform,
    ...
    out_of_box_atoms = tf.squeeze(
        tf.reduce_sum(
            tf.cast(tf.square(box_size * 0.5) - tf.cast(tf.square(rotatated_ligand_coords), tf.float32) < 0, tf.int32),
            reduction_indices=-1))
    ...
    ceiled_receptor_coords = tf.cast(
        tf.round((tf.constant(-0.5, tf.float32) + (tf.cast(side_pixels, tf.float32) / 2.0) + (
        rotated_receptor_coords / pixel_size)) * epsilon),
        tf.int64)
    ...
    multiple_cropped_receptor_coords = tf.ones([tf.shape(ceiled_ligand_coords)[0], 1, 1],
                                               tf.int64) * cropped_receptor_coords
    ...
    multiple_complex_elements = tf.reshape(base * tf.reshape(complex_elements, [1, tf.shape(complex_elements)[0], 1]),
                                           [-1])
